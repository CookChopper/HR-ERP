...
import { getUserPersonlById, getUserJob } from '@/api/user'
export default {
  data() {
    return {
      formData: {},
  ...
  methods: {
    async getPersonalDetail() {
      this.formData = await getUserPersonlById(this.userId) // 获取个人基本信息
    },
    async getJobDetail() {
      const userInfo = await getUserPersonlById(this.userId)
      const jobInfo = await getUserJob(this.userId) // 获取个人基本信息
