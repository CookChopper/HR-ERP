      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <image-upload ref="staffPhoto" :handle-remove="handleRemoveStaffPhoto" :on-success="onSuccessStaffPhoto" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <image-upload ref="myStaffPhoto" :handle-remove="handleRemoveMyStaffPhoto" :on-success="onSuccessMyStaffPhoto" />
        </el-form-item>
...
import { getUserDetailById, saveUserDetailById, getUserPersonlById, saveUserPersonl } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    // 保存个人基本信息
    async saveUser() {
      if (!this.$refs.staffPhoto.uploadStatus) {
        return this.$message.info('请等待图片上传后再保存更信息')
      }
      await saveUserDetailById({ ...this.userInfo, password: '' })
      this.$message.success('操作成功')
      this.getUserDetailById()
    },
    // 获取个人基础信息
    async getUserPersonlById() {
      this.formData = await getUserPersonlById(this.userId)
      this.$refs.myStaffPhoto.fileList = [{ url: this.formData.staffPhoto }]
    },
    // 保存个人基础信息
    async savePersonal() {
      await saveUserPersonl({ ...this.formData, id: this.userId })
      this.$message.success('操作成功')
    },
    // 删除图片操作
    async handleRemoveStaffPhoto(file, fileList) {
      // 方法1
      this.$refs.staffPhoto.fileList = fileList
      await saveUserDetailById({ ...this.userInfo, password: '', staffPhoto: ' ' })
    ...
    // 删除图片操作
    handleRemoveMyStaffPhoto(file, fileList) {
      // 方法1
      this.$refs.myStaffPhoto.fileList = fileList
      // console.log(fileList)
      // 方法2
      // this.fileList = this.fileList.filter(item => item.uid !== file.uid)
    },
    // 上传员工头像成功回调函数
    async onSuccessStaffPhoto(url) {
      // console.log(url)
      await saveUserDetailById({ ...this.userInfo, password: '', staffPhoto: url })
      if (this.userId === this.$store.getters.userId) {
        await this['user/getUserInfo']()
      }
    ...
    // 上传员工照片成功回调函数
    async onSuccessMyStaffPhoto(url) {
      await saveUserPersonl({ ...this.formData, id: this.userId, staffPhoto: url })
      this.$message.success('图片上传成功')
    }
  }
}
